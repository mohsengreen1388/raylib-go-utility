...
func (li *Lights) SpotLightInit(lightRadius, ambientStrong float32, ambientColor rl.Color) {
	li.configShader()
...
func (li *Lights) UpdatePosition(pos rl.Vector2){
	rl.SetShaderValue(*li.Shader,li.posLoc, []float32{pos.X,pos.Y}, rl.ShaderUniformVec2)
}

func (li *Lights) UpdateOptions(lightRadius, ambientStrong float32, ambientColor rl.Color){
	li.LightRadius = lightRadius
	li.AmbientStrong = ambientStrong
	li.AmbientColor = ambientColor
...
func (li *Lights) PointLight(texture *rl.Texture2D, position rl.Vector2, scale float32, color []color.RGBA) {
