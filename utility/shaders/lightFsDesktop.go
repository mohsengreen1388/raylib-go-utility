...
const LightFsDesktop = `#version 330
...
void main()
{
    ...
    vec3 ambient = ambientStrength*ambientColor*vec3(texture(texture0,fragTexCoord));
    vec3 result = vec3(0);
...
vec3 CalcPointLight(Light light,vec3 normal,vec3 viewDir,vec3 ambient)
{
    ...
    vec3 CalcSpotLight(Light light,vec3 normal,vec3 viewDir,vec3 ambient)
    {
        ...
        intensity*=length(vec3(texture(flashlight,(fragTexCoord))));
        diffuse*=intensity;
        specular*=intensity;
