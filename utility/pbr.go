...
func (ph *PhysicRender) configShader() {
	if checkPlatformIsMobail() {
		ph.Shader = ptr(rl.LoadShaderFromMemory(ShaderFile.Mobail["pbrVs"], ShaderFile.Mobail["pbrFs"]))
	} else {
		ph.Shader = ptr(rl.LoadShaderFromMemory(ShaderFile.Desktop["pbrVs"], ShaderFile.Desktop["pbrFs"]))
	}
}
